...
export default function ApplicationsKeys(){
  ...
  }catch(e){ const msg = (e as unknown) instanceof Error ? (e as Error).message : String(e); setError(msg) }
    finally{ setLoading(false) }
  }

  useEffect(() => { load() }, [])

  function openCreate(){ setCreateForm({ name: '', expiration: '', neverExpires: false, permissions: { createBucket: false } }); setCreateOpen(true) }
  ...
  async function submitCreate(){
    ...
    try{
      ...
      await load()
      setCreateOpen(false)
    }catch(e){ console.error('CreateKey error', e); setError(String(e)) }finally{ setCreateSubmitting(false) }
  }

  function openImport(){ setImportForm({ accessKeyId: '', secretAccessKey: '', name: '' }); setImportOpen(true) }
  ...
  async function submitImport(){
    ...
    try{
      ...
      await load()
      setImportOpen(false)
  }catch(e){ console.error('ImportKey error', e); setError(String(e)) }
  ...
  async function doDelete(){
    ...
    try{
      ...
      await load()
  }catch(e){ console.error('DeleteKey error', e); setError(String(e)) }
  finally{ setDeleteDialogOpen(false); setToDeleteId(null) }
  }
  ...
  setDetailsForm({ name: res?.name || '', expiration: res?.expiration || '', neverExpires: false, permissions: { createBucket: !!res?.permissions?.createBucket } })
      setDetailOpen(true)
    }catch(e){ console.error('GetKeyInfo error', e); setSelectedKey(null); setError(String(e)) }
  }

  function closeDetails(){ setDetailOpen(false); setSelectedKey(null); setEditing(false) }
  ...
  const refreshed = await GetKeyInfo({ id: selectedKey.accessKeyId })
      ...
      await load()
      setEditing(false)
  }catch(e){ console.error('UpdateKey error', e); setError(String(e)) }
    ...
    <Box>
      ...
      <Box sx={{display:'flex',justifyContent:'space-between',alignItems:'center',mb:2}}>
        <Stack>
          ...
          <Typography variant="body2" color="text.secondary">{t('dashboard.apps_desc')}</Typography>
        </Stack>
        <Stack direction="row" spacing={1}>
          <Button variant="outlined" onClick={() => load()}>{t('common.refresh')}</Button>
          <Button variant="contained" onClick={openCreate}>{t('common.add')}</Button>
          <Button variant="text" onClick={openImport}>{t('keys.import.label')}</Button>
        </Stack>
      </Box>
        ...
        <Table size="small">
          ...
          <TableBody>
            ...
            {!loading && keys.length === 0 && (
              <TableRow>
                <TableCell colSpan={4} sx={{textAlign:'center'}}>{t('keys.empty')}</TableCell>
              </TableRow>
            )}
            {!loading && keys.map((k) => (
              <TableRow key={k.id} hover>
                ...
                <TableCell>
                  <Stack direction="row" spacing={1} alignItems="center">
                    <Button size="small" onClick={() => openDetails(k.id)}>{t('common.details')}</Button>
                    <IconButton size="small" aria-label="delete" onClick={() => confirmDelete(k.id)}>
                      <span style={{color:'#d32f2f'}}>{t('common.delete')}</span>
                    </IconButton>
                  </Stack>
                </TableCell>
              </TableRow>
      ...
      {/* Create dialog */}
        ...
        <DialogContent>
          <Stack spacing={2} sx={{mt:1}}>
            ...
            <TextField label={t('keys.create_expiration_label')} type="datetime-local" value={createForm.expiration} onChange={(e) => setCreateForm(f => ({...f, expiration: e.target.value}))} fullWidth InputLabelProps={{shrink:true}} />
            <label>
              <input type="checkbox" checked={createForm.permissions.createBucket} onChange={(e) => setCreateForm(f => ({...f, permissions: {...f.permissions, createBucket: e.target.checked}}))} /> {t('keys.perm.createBucket')}
            </label>
            <label>
        ...
        <DialogActions>
          ...
          <Button variant="contained" onClick={submitCreate} disabled={createSubmitting}>{t('common.add')}</Button>
        </DialogActions>
      </Dialog>
      ...
      <Dialog open={importOpen} onClose={closeImport} fullWidth maxWidth="sm">
        ...
        <DialogContent>
          <Stack spacing={2} sx={{mt:1}}>
            ...
            <TextField label={t('keys.import.secretAccessKey')} value={importForm.secretAccessKey} onChange={(e) => setImportForm(f => ({...f, secretAccessKey: e.target.value}))} fullWidth />
        ...
        <DialogActions>
          ...
          <Button variant="contained" onClick={submitImport} disabled={importSubmitting}>{t('keys.import.label')}</Button>
        </DialogActions>
      </Dialog>
      ...
      <Dialog open={detailOpen} onClose={closeDetails} fullWidth maxWidth="md">
        ...
        <DialogContent>
          {selectedKey ? (
            <Box sx={{mt:1}}>
              {!editing ? (
                <Box>
                  <Typography><b>{t('keys.col.name')}:</b> {selectedKey.name}</Typography>
                  <Typography><b>{t('keys.id_label')}:</b> {selectedKey.accessKeyId}</Typography>
                  <Typography><b>{t('keys.created_label')}:</b> {selectedKey.created ?? ''}</Typography>
                  <Typography><b>{t('keys.expiration_label')}:</b> {selectedKey.expiration ?? t('common.never_expire')}</Typography>
                  <Typography><b>{t('keys.expired_label')}:</b> {selectedKey.expired ? t('common.yes') : t('common.no')}</Typography>
                  <Typography><b>{t('keys.permissions_label')}:</b></Typography>
                  <Box sx={{mt:0.5}}>
                    {selectedKey.permissions && Object.keys(selectedKey.permissions).length > 0 ? (
                      <Stack direction="row" spacing={1} sx={{flexWrap:'wrap'}}>
                        {Object.entries(selectedKey.permissions).map(([k,v]) => (
                          v ? <Chip key={k} label={t(`keys.perm.${k}`)} size="small" /> : null
                        ))}
                      </Stack>
                    ) : (
                      <Typography variant="body2" color="text.secondary">{t('keys.no_permissions')}</Typography>
                    )}
                  </Box>
                  <Typography><b>{t('keys.buckets_label')}:</b> {(selectedKey.buckets || []).map(b => b.id).join(', ')}</Typography>
                  <Box sx={{mt:1}}>
                    {selectedKey.secretAccessKey ? (
                      <Box>
                        {!showSecret ? (
                          <Button variant="outlined" size="small" onClick={() => setShowSecret(true)}>{t('keys.show_secret')}</Button>
                        ) : (
                          <Box sx={{display:'flex',gap:1,alignItems:'center'}}>
                            <code style={{wordBreak:'break-all'}}>{selectedKey.secretAccessKey}</code>
                            <Button size="small" onClick={() => { navigator.clipboard?.writeText(selectedKey.secretAccessKey || ''); }}>{t('common.copy')}</Button>
                            <Button size="small" onClick={() => setShowSecret(false)}>{t('keys.hide_secret')}</Button>
                          </Box>
                        )}
                      </Box>
                    ) : (
                      <Typography variant="body2" color="text.secondary">{t('keys.no_secret')}</Typography>
                    )}
                  </Box>
                </Box>
              ) : (
                <Stack spacing={2}>
                  ...
                  {!detailsForm.neverExpires && (
                    <TextField label={t('adminTokens.expiration') as string} type="datetime-local" value={detailsForm.expiration || ''} onChange={(e) => setDetailsForm(d => ({...d, expiration: e.target.value}))} InputLabelProps={{shrink:true}} />
                  )}
                  <label>
                    <input type="checkbox" checked={detailsForm.neverExpires} onChange={(e) => setDetailsForm(d => ({...d, neverExpires: e.target.checked}))} /> {t('common.never_expire')}
                  </label>
                  <label>
                    <input type="checkbox" checked={detailsForm.permissions.createBucket} onChange={(e) => setDetailsForm(d => ({...d, permissions: {...d.permissions, createBucket: e.target.checked}}))} /> {t('keys.perm.createBucket')}
                  </label>
                </Stack>
              )}
            </Box>
        ...
        <DialogActions>
          ...
          {editing && <Button onClick={() => { setEditing(false); setDetailsForm({ name: selectedKey?.name || '', expiration: selectedKey?.expiration || '', neverExpires: false, permissions: { createBucket: !!selectedKey?.permissions?.createBucket } }) }}>{t('common.cancel')}</Button>}
          {editing && <Button variant="contained" onClick={saveDetails} disabled={savingDetails}>{t('common.save')}</Button>}
          <Button onClick={closeDetails}>{t('common.close')}</Button>
        </DialogActions>
      </Dialog>

      {/* Created secret dialog (shown after create/import if API returns secret) */}
      <Dialog open={createdSecretOpen} onClose={() => { setCreatedSecretOpen(false); setCreatedSecret(null) }}>
        ...
        <DialogActions>
          <Button onClick={() => { navigator.clipboard?.writeText(createdSecret || ''); }}>{t('common.copy')}</Button>
          <Button onClick={() => { setCreatedSecretOpen(false); setCreatedSecret(null) }}>{t('common.close')}</Button>
        </DialogActions>
      </Dialog>
      ...
      <Dialog open={deleteDialogOpen} onClose={() => setDeleteDialogOpen(false)}>
        ...
        <DialogActions>
          ...
          <Button color="error" variant="contained" onClick={doDelete}>{t('common.delete')}</Button>
        </DialogActions>
      </Dialog>
    </Box>
  )
