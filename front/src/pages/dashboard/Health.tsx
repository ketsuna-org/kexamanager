...
export default function Health() {
    ...
    useEffect(() => {
        ...
        GetClusterStatistics()
            ...
            .catch((e) => {
                if (mounted) setStatsError((e as unknown as { message?: string })?.message || String(e))
            })
            .finally(() => {
                ...
                <Box>
                    ...
                    <Box sx={{ display: "flex", gap: 3, flexWrap: "wrap", alignItems: "stretch" }}>
                        ...
                        <Box sx={{ flex: "1 1 220px", minWidth: 220 }}>
                            <MetricPaper
                                ...
                                label={t("dashboard.partitions_quorum")}
                                value={`${health.partitionsQuorum} / ${health.partitions}`}
                            />
                        </Box>
                    </Box>
                        ...
                        {!statsLoading && !statsError && stats && (
                            <Paper variant="outlined" sx={{ p: 2, bgcolor: "background.paper" }}>
                                ...
                                <Box display="flex" justifyContent="flex-end" gap={1}>
                                    <Button size="small" onClick={copyStats} startIcon={<ContentCopyIcon />}>
                    ...
                    <Snackbar open={snackOpen} autoHideDuration={2500} onClose={() => setSnackOpen(false)} anchorOrigin={{ vertical: "bottom", horizontal: "right" }}>
                        <Alert severity="success" sx={{ width: "100%" }} onClose={() => setSnackOpen(false)}>
                            {t("common.copied") ?? t("dashboard.copy_json")}
                        </Alert>
                    </Snackbar>
                </Box>
            )}
