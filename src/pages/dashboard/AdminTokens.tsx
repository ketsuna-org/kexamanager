...
export default function AdminTokens(){
  ...
  function openEdit(tok: AdminToken){
    if(!tok.id){ setSnack({open:true,severity:'error',message:t('adminTokens.invalid_id')}); return }
  ...
  async function handleSubmit(){
    ...
    try{
      if(editing && editing.id){
        await UpdateAdminToken({ id: editing.id }, {
  ...
  async function handleDelete(){
    if(!current) return
    if(!current.id){ setSnack({open:true,severity:'error',message:t('adminTokens.invalid_id')}); return }
    setBusy(true)
    try{
      ...
      await load()
    }catch(e){ const msg = e instanceof Error ? e.message : String(e); setSnack({open:true,severity:'error',message:msg}) }
    finally{ setBusy(false) }
  }
  ...
  return (
    <div>
      <Stack direction="row" justifyContent="space-between" alignItems="center" sx={{mb:2}}>
        ...
        <div>
          <Button variant="contained" onClick={openCreate}>{t('adminTokens.token_add')}</Button>
        </div>
      </Stack>

      {loading && <div><CircularProgress size={20}/> {t('common.loading')}</div>}
        ...
        <TableContainer component={Paper}>
          <Table size="small">
            ...
            <TableBody>
              {tokens.length === 0 && (
                <TableRow>
                  <TableCell colSpan={7} sx={{textAlign:'center'}}>{t('adminTokens.empty')}</TableCell>
                </TableRow>
              )}
              {tokens.map((tok, idx) => {
                return (
                  <TableRow key={tok.id ?? idx}>
                    ...
                    <TableCell>
                      <Stack direction="row" spacing={1}>
                        <Tooltip title={t('adminTokens.view') as string}><span>
                          <IconButton size="small" onClick={() => openView(tok)}><VisibilityIcon fontSize="small"/></IconButton>
                        </span></Tooltip>
                        <Tooltip title={t('adminTokens.edit') as string}><span>
                          <IconButton size="small" onClick={() => openEdit(tok)}><EditIcon fontSize="small"/></IconButton>
                        </span></Tooltip>
                        <Tooltip title={t('adminTokens.delete') as string}><span>
                          <IconButton size="small" color="error" onClick={() => openConfirm(tok)}><DeleteIcon fontSize="small"/></IconButton>
                        </span></Tooltip>
                      </Stack>
                    </TableCell>
                  </TableRow>
      ...
      <Dialog open={formOpen} onClose={() => setFormOpen(false)} fullWidth maxWidth="sm">
        ...
        <DialogContent>
          <Stack spacing={2} sx={{mt:1}}>
            ...
            <TextField label={t('adminTokens.scope') as string} value={form.scope} onChange={e=>setForm(s=>({...s,scope:e.target.value}))} helperText={t('adminTokens.scope_desc') as string} fullWidth />
            <FormControlLabel control={<Checkbox checked={form.neverExpires} onChange={e=>setForm(s=>({...s,neverExpires:e.target.checked}))}/>} label={t('common.never_expire') as string} />
            {!form.neverExpires && (
              <TextField label={t('adminTokens.expiration') as string} type="datetime-local" value={form.expiration || ''} onChange={e=>setForm(s=>({...s,expiration:e.target.value}))} InputLabelProps={{shrink:true}} />
            )}
          </Stack>
        </DialogContent>
        <DialogActions>
          <Button onClick={() => setFormOpen(false)}>{t('common.cancel')}</Button>
          <Button variant="contained" onClick={handleSubmit} disabled={busy}>{busy ? <CircularProgress size={16}/> : t('common.save')}</Button>
        </DialogActions>
      </Dialog>
      ...
      <Dialog open={confirmOpen} onClose={() => setConfirmOpen(false)}>
        ...
        <DialogContent>
          {t('adminTokens.delete_confirm_desc')}
        </DialogContent>
        <DialogActions>
          <Button onClick={() => setConfirmOpen(false)}>{t('common.cancel')}</Button>
          <Button color="error" variant="contained" onClick={handleDelete} disabled={busy}>{busy ? <CircularProgress size={16}/> : t('common.delete')}</Button>
        </DialogActions>
      </Dialog>
      ...
      <Dialog open={createdSecretOpen} onClose={() => { setCreatedSecretOpen(false); setCreatedSecret(null) }}>
        ...
        <DialogActions>
          <Button onClick={() => { navigator.clipboard?.writeText(createdSecret || ''); setSnack({open:true,severity:'success',message:t('common.copied')}) }}>{t('common.copy')}</Button>
