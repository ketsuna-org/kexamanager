...
export default function Workers(){
  ...
  const res = await ListWorkers({}, {})
      ...
      else if (Array.isArray(maybe)) setWorkers(maybe as unknown[])
      else setWorkers([])
  ...
  return (
    <div>
      <h3>{t('dashboard.workers')}</h3>
      <p>{t('dashboard.workers_desc')}</p>
        ...
        <TableContainer component={Paper}>
          <Table size="small">
            ...
            <TableBody>
              {workers.length === 0 && (
                <TableRow>
                  <TableCell colSpan={3} sx={{textAlign:'center'}}>{t('workers.empty')}</TableCell>
                </TableRow>
              )}
              {workers.map((w, idx) => {
                const obj = w as unknown as Record<string, unknown>
                const label = (typeof obj['Name'] === 'string' && obj['Name']) || (typeof obj['Worker'] === 'string' && obj['Worker']) || JSON.stringify(obj)
                return (
                  <TableRow key={idx}>
                    <TableCell>{idx+1}</TableCell>
                    <TableCell>{label}</TableCell>
                    <TableCell>
                      <Button size="small" onClick={() => openDetails(w)}>{t('common.details')}</Button>
