...
export default function ApplicationsKeys(){
  ...
  async function load(){
    ...
    try{
      ...
      else if (Array.isArray(maybe)) setKeys(maybe as unknown[])
      else setKeys([])
  ...
  return (
    <div>
      <h3>{t('dashboard.apps')}</h3>
      <p>{t('dashboard.apps_desc')}</p>
        ...
        <TableContainer component={Paper}>
          <Table size="small">
            ...
            <TableBody>
              {keys.length === 0 && (
                <TableRow>
                  <TableCell colSpan={3} sx={{textAlign:'center'}}>{t('keys.empty')}</TableCell>
                </TableRow>
              )}
              {keys.map((k, idx) => {
                const obj = k as unknown as Record<string, unknown>
                const label = (typeof obj['Name'] === 'string' && obj['Name']) || (typeof obj['Key'] === 'string' && obj['Key']) || JSON.stringify(obj)
                return (
                  <TableRow key={idx}>
                    <TableCell>{idx+1}</TableCell>
                    <TableCell>{label}</TableCell>
                    <TableCell>
                      <Button size="small" onClick={() => openDetails(k)}>{t('common.details')}</Button>
