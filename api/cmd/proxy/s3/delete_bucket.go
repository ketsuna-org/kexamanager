...
// HandleDeleteBucketWithConfig handles the delete bucket request with pre-validated config
func HandleDeleteBucketWithConfig(config S3ConfigData) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		if r.Method != http.MethodPost {
			http.Error(w, "Method not allowed", http.StatusMethodNotAllowed)
			return
